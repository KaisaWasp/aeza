      ...
      <table className="w-full border-collapse text-left">
        <thead>
          <tr className="border-b-2 border-[#1C202A]">
            ...
            <th className="py-2 px-3 text-[#FF375F] border-r-2 border-[#1C202A]">RTT min / avg / max</th>
            <th className="py-2 px-3 text-[#FF375F]">IP Address</th>
          </tr>
        </thead>
        <tbody>
          {taskResult.tasksToAgents.map((task: any, index: number) => {
            const rtt = task.result?.rtt
              ? `${task.result.rtt.min} / ${task.result.rtt.avg} / ${task.result.rtt.max}`
              : '-';
